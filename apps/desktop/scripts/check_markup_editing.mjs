...
const markup = await readFile(join(root, 'src-tauri/src/markup_transaction.rs'), 'utf8');
...
for (const token of [
  'MAX_MARKUP_FILES', 'MAX_FILE_BYTES', 'MAX_TOTAL_BYTES',
  'selection.id_unique', 'direct_dom_tag', 'literal JSX/TSX DOM owner',
  'literal-tailwind-utility', 'responsive/state Tailwind variant', 'theme/config/runtime semantics',
  'jsx-inline-style-literal', 'inline style object contains a spread',
  'file_fingerprint', 'fs::symlink_metadata', 'canonical.starts_with(&root)',
  '.create_new(true)', 'file.sync_all()', 'fs::rename(&temp, path)',
  'Updated JSX/TSX failed bounded opening-tag structural validation',
]) {
  if (!markup.includes(token)) throw new Error(`Native markup transaction missing ${token}`);
}
...
for (const token of [
  ...
  if (!properties.includes(token)) throw new Error(`Properties markup UX missing ${token}`);
}
...
console.log('Monument Tailwind + JSX source editing contract: PASS');
