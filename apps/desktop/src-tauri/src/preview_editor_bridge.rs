...
impl PreviewEditorMessage {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if encoded.len() > limit {
            return Err(format!(
                "Preview editor {} payload exceeded the {} byte boundary",
                self.kind, limit
            ));
        }
        Ok(())
...
fn editor_eval(app: &AppHandle, expression: String) -> Result<(), String> {
    let webview = app
        ...
        .ok_or_else(|| "Preview webview is not open".to_string())?;
...
pub fn preview_editor_emit(
    ...
    message.validate()?;
    {
        let mut runtime = state
            .lock()
            .map_err(|_| "Preview editor bridge lock poisoned".to_string())?;
    ...
    if message.kind == "selection" {
        ...
        app.emit("monument://preview-selection", selection)
...
pub fn preview_editor_request_tree(app: AppHandle) -> Result<(), String> {
    editor_eval(
        &app,
        "window.__MONUMENT_EDITOR__ && window.__MONUMENT_EDITOR__.requestTree();".into(),
    )
}
...
pub fn preview_editor_select(app: AppHandle, node_id: String) -> Result<(), String> {
    ...
    editor_eval(
        &app,
        format!("window.__MONUMENT_EDITOR__ && window.__MONUMENT_EDITOR__.select({encoded});"),
    )
}
...
pub fn preview_editor_hover(app: AppHandle, node_id: Option<String>) -> Result<(), String> {
    ...
    editor_eval(
        &app,
        format!("window.__MONUMENT_EDITOR__ && window.__MONUMENT_EDITOR__.hover({encoded});"),
    )
}

#[cfg(test)]
mod tests {
    use super::{valid_node_id, PreviewEditorMessage, MAX_TREE_BYTES};
    use serde_json::json;
    ...
    fn editor_bridge_payloads_are_bounded() {
        ...
        let oversized = PreviewEditorMessage {
            kind: "tree".into(),
            payload: json!({"blob": "x".repeat(MAX_TREE_BYTES + 1)}),
        };
