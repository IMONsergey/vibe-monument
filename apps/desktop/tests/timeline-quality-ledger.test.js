...
test('Timeline persists deterministic and browser quality by code generation', async () => {
  ...
  assert.ok(verification.includes('requestAutoRepairIfEnabled(evidence)'), 'quality recording must coexist with bounded Auto Repair');
  assert.ok(verification.indexOf('await persistFinal(evidence)') < verification.indexOf('requestAutoRepairIfEnabled(evidence)'), 'quality/evidence must persist before repair is requested');
