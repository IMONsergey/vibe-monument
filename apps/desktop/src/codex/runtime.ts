...
export class CodexRuntime {
  ...
  constructor() {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.addEventListener(AUTO_REPAIR_EVENT, this.handleRepairEvent);
    }
  }
  ...
  private readonly handleRepairEvent = (event: Event) => {
    const request = (event as CustomEvent<AutoRepairRequest>).detail;
    if (!request) return;
    void this.handleRepairRequest(request);
  };

  private async handleRepairRequest(request: AutoRepairRequest): Promise<void> {
    ...
    if (request.source === 'automatic') {
      ...
      attempt = attempts + 1;
      this.autoRepairAttempts.set(request.projectRoot, attempt);
    }

    this.handledRepairEvidence.add(request.evidenceId);
    try {
      await this.sendRepair(request, attempt);
    } catch (error) {
      this.activity('error', 'Repair could not start', String(error instanceof Error ? error.message : error));
    }
  }

  private async sendRepair(request: AutoRepairRequest, attempt: number | null): Promise<void> {
    if (!this.connected || this.snapshot.state !== 'ready') throw new Error('Codex is not ready for repair');
    ...
    if (!threadId) {
      ...
      const record = recordOf(created);
      const thread = asThread(record.thread ?? created);
      if (!thread) throw new Error('Codex did not return a thread id for repair');
      threadId = thread.id;
      this.patch({ threads: [thread, ...this.snapshot.threads], activeThreadId: thread.id });
    }
    this.patch({ state: 'busy', message: '', approval: null });
    const title = request.source === 'automatic' && attempt != null
      ? `Auto repair ${attempt}/${MAX_AUTO_REPAIR_ATTEMPTS}`
      : request.label || 'Fix with Monument';
    this.activity('thinking', title, request.source === 'automatic' ? 'Fixing deterministic verification failures with bounded evidence.' : 'Repairing the selected evidence with bounded context.');
