...
async function openNativePreview(url: string, bounds: PreviewBounds): Promise<void> {
  await invokeNative<void>('preview_open', { url, bounds });
}

async function setNativePreviewBounds(bounds: PreviewBounds): Promise<void> {
  await invokeNative<void>('preview_set_bounds', { bounds });
}

async function setInspect(enabled: boolean): Promise<void> {
  await invokeNative<void>('preview_set_inspect', { enabled });
}

async function closePreview(): Promise<void> {
  await invokeNative<void>('preview_close');
}
...
export function NativePreview({
  ...
  const syncBounds = useCallback(() => {
    ...
    if (!bounds) return;
    void setNativePreviewBounds(bounds).catch((error) => onError(String(error instanceof Error ? error.message : error)));
  }, [native, onError]);
  ...
  useLayoutEffect(() => {
    ...
    const open = async () => {
      ...
      if (!bounds) return;
      try {
        await openNativePreview(url, bounds);
        if (disposed) {
        ...
        disposers.push(await listenNative<PreviewSelection>('monument://preview-selection', (next) => {
          ...
          void setInspect(false).catch(() => undefined);
        }));
        disposers.push(await listenNative<string>('monument://preview-error', (message) => onError(message)));
      } catch (error) {
    ...
    return () => {
      ...
      void closePreview().catch(() => undefined);
    };
  ...
  if (!native) {
    return <iframe src={url} title="Project preview" allow="clipboard-read; clipboard-write" />;
  }

  return (
    <div className="native-preview-wrap">
      <div className="preview-local-toolbar">
        <button type="button" className={inspect ? 'active' : ''} onClick={() => void toggleInspect()}>
          {inspect ? 'Selecting…' : 'Select'} <kbd>I</kbd>
        </button>
        {selection ? (
