...
import { CodexRuntimeController } from './codex/runtime-controller.js';
import { createCodexTransport } from './host/codex-transport.js';
...
function render() {
  ...
  app.innerHTML = `<div class="app-shell">
    <header class="titlebar" data-tauri-drag-region><div class="titlebar-left"><div class="traffic"><span></span><span></span><span></span></div><div class="brand">Monument</div></div>
      ...
      <div class="titlebar-right"><span class="status-pill ${state.codexStatus === 'error' ? 'error' : ''}"><span class="status-dot"></span>${state.codexStatus === 'connected' ? `Codex ${state.codexMode === 'native' ? 'connected' : 'demo'}` : state.codexStatus === 'error' ? 'Codex unavailable' : 'Connecting Codex'}</span><button class="icon-button" data-open-palette>${icons.command}</button><button class="icon-button" data-toast="Settings">${icons.settings}</button></div></header>
    <div class="workspace">${sidebarMarkup(state)}<main class="main ${state.bottomOpen ? '' : 'bottom-closed'}"><div class="workspace-toolbar"><div class="segmented"><button class="segment ${state.centerMode === 'preview' || state.centerMode === 'inspect' ? 'active' : ''}" data-mode="preview">Preview</button><button class="segment ${state.centerMode === 'code' ? 'active' : ''}" data-mode="code">Code</button></div><div class="url-pill">${icons.globe}<span>localhost:5173</span>${icons.refresh}</div><div class="toolbar-spacer"></div><div class="viewport-toggle"><button class="icon-button ${state.previewViewport === 'desktop' ? 'active' : ''}" data-viewport="desktop">${icons.desktop}</button><button class="icon-button ${state.previewViewport === 'mobile' ? 'active' : ''}" data-viewport="mobile">${icons.mobile}</button></div><button class="icon-button ${state.centerMode === 'inspect' ? 'active' : ''}" data-mode="inspect" title="Inspect live UI">${icons.layers}</button><button class="icon-button" data-toast="Open preview in browser">${icons.external}</button>${!state.bottomOpen ? `<button class="icon-button" data-open-bottom>${icons.panel}</button>` : ''}</div>
      <div class="canvas-wrap">${previewMarkup(state)}</div>${bottomPanelMarkup(state)}</main>${agentMarkup(state)}</div></div>${paletteMarkup(state, commands)}<div class="toast"></div>`;
  bindEvents();
}
...
function bindEvents() {
  ...
  document.querySelector('[data-send]')?.addEventListener('click', async () => {
    ...
    if (!text) return showToast('Describe the change first');
    try {
      await codexRuntime?.sendText(state.activeThreadId, text);
      ta.value = '';
      showToast('Turn sent to Codex');
    } catch (error) {
      showToast(`Codex: ${error?.message ?? error}`);
    }
  });
...
async function connectCodex() {
  const transport = createCodexTransport(globalThis);
  const client = new CodexAppServerClient(transport);
  codexRuntime = new CodexRuntimeController(client, {
    mode: transport.kind,
    onStatus: ({ status, mode }) => setState({ codexStatus: status, codexMode: mode }),
    onThreads: (threads) => setState({ codexThreads: threads, activeThreadId: threads[0]?.id ?? state.activeThreadId }),
    onProjection: (projection) => {
      if (projection.message !== state.codexMessage) setState({ codexMessage: projection.message });
    },
  });
  try { await codexRuntime.connect(); } catch (error) { console.error(error); }
}
